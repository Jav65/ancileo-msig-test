document.addEventListener("DOMContentLoaded", () => {
  ...
  const appendToolRuns = (toolRuns) => {
    ...
    toolRuns.forEach((run) => {
      ...
      const name = run?.name || run?.tool || "unknown";
  ...
  form.addEventListener("submit", async (event) => {
    ...
    if (!text) {
      setStatus("Ask something about insurance or coverage.");
      return;
    }
    ...
    try {
      const response = await fetch("/gmail/chat/send", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        ...
        appendMessage("assistant", `Warning: ${detail}`);
      ...
      console.error(error);
      appendMessage("assistant", "Warning: Unable to reach the assistant. Please try again.");
